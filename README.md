...
Preview details decode complete High LOD Main meshes through a bounded MDL V5/V6 parser after selection debounce. The parser reads positions, UVs, vertex colors, material references, bone tables, skinning weights, and indices, validates every range, and avoids the later V6 sections that fail in the bundled Lumina parser. Human shared equipment is transformed to the selected race and body type with the game's `human.pbd` bone deformers; Young NPC parts resolve through their verified Young and shared Human ancestor models, NPC-only Face values also try their normalized model ID, and missing race/gender Hands or Feet models can fall back to shared `c0101` parts before the verified bare `e0000` fallback. The renderer resolves IMC material variants, loads game MTRL and TEX assets, composes character color tables, applies both legacy and Dawntrail STM staining templates for the selected equipment dyes, applies Human skin color through `skin.shpk` influence maps, and builds Human hair color from the selected NPC's `human.cmp` customization palette. Demihuman hair uses each MTRL's `g_DiffuseColor`, including hair geometry stored outside the Head model, and supports the character mask sampler used by `_s.tex` assets. Missing normals are generated from triangle geometry, while malformed parts are skipped and reported. No native game object is created. The static 3D preview can be disabled in Settings; disabling it releases the active scene and generated textures and skips geometry inspection.
