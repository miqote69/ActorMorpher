...
Preview details decode complete High LOD Main meshes through a bounded MDL V5/V6 parser after selection debounce. The parser reads positions, UVs, vertex colors, material references, attributes, submeshes, bone tables, skinning weights, and indices, validates every range, and avoids the later V6 sections that fail in the bundled Lumina parser. Human shared equipment is transformed to the selected race and body type with the game's `human.pbd` bone deformers; Young NPC parts resolve through their verified Young and shared Human ancestor models, NPC-only Face values also try their normalized model ID, and missing race/gender equipment prefers a same-race model only when the PBD graph confirms that it can deform to the selected target before trying broader shared fallbacks. Face preview geometry filters `atr_fv_*` submeshes by the selected FacialFeature bitmask instead of drawing mutually exclusive feature parts together. The renderer resolves each IMC material variant together with its ten-bit submesh mask, filters `atr_*v_a` through `atr_*v_j`, and selects `atr_tls` or `atr_tlh` geometry from the target race's tail support. It then loads game MTRL and TEX assets, composes character color tables, applies both legacy and Dawntrail STM staining templates for the selected equipment dyes, and obtains Human skin, hair, iris, and facial-feature colors from the selected NPC's `human.cmp` customization palette. MTRL shader flags control backface visibility; two-sided lining is shaded toward the camera and drawn before outward-facing surfaces to prevent it from leaking through the outfit during rotation. Human overlap ordering places body `skin.shpk` deepest, lower-body `dwn` equipment next, and outer `top` equipment last, while source vertex normals provide smoothly interpolated lighting instead of triangle-faceted highlights. PBD-deformed shared equipment regenerates smooth normals from its transformed geometry. `iris.shpk`, `charactertattoo.shpk`, and character occlusion materials receive preview-specific texture handling. Demihuman hair uses each MTRL's `g_DiffuseColor`, including hair geometry stored outside the Head model, and supports the character mask sampler used by `_s.tex` assets. Missing normals are generated from triangle geometry, while malformed parts are skipped and reported. No native game object is created. The static 3D preview can be disabled in Settings; disabling it releases the active scene and generated textures and skips geometry inspection.
