...
Model Search now resolves and displays preview asset readiness for Human, Demihuman, and Monster entries. Demihuman and Monster MDL geometry is rendered as a bounded static software 3D preview with drag rotation, wheel zoom, and camera reset. Human preview inputs are validated into immutable Customize, equipment, weapon, and visibility data, but native CharaView allocation remains disabled because current APIs do not expose exclusive slot ownership or verified native texture lifetime rules. See [preview research](docs/MODEL_PREVIEW_RESEARCH.md) and [preview architecture](docs/MODEL_PREVIEW.md).
