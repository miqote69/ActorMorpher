...
public sealed unsafe class NativeDrawObjectInjectorTests
{
    ...
    [Theory]
    [MemberData(nameof(IncompleteCurrentAppearances))]
    public void PersistentTransferDoesNotPrepareInjectionForIncompleteLatestPublication(AppearanceData? latest)
    {
        var transition = new LocalPlayerAppearancePersistence();
        var source = Field(10);
        var destination = Cutscene(11);
        transition.RecordApplied(Key(10), source, 1);
        transition.UpdatePublishedAppearance(source, true, 2);
        ActorRepresentationKey? requested = null;

        var prepared = NativeDrawObjectInjector.TryPreparePersistentTransfer(
            transition,
            destination,
            key =>
            {
        ...
        Assert.False(transition.IsArmed);
        Assert.Equal(source, transition.CurrentRepresentation);
        Assert.False(transition.TryGetArmedSource(out _, out _));
    }

    [Theory]
    [MemberData(nameof(CompletePublishedAppearances))]
    public void PersistentTransferUsesTheExactLatestPublishedAppearanceWithoutACopy(AppearanceData latest)
    {
        var transition = new LocalPlayerAppearancePersistence();
        var source = Field(10);
        var destination = GPose(11);
        var selectedB = Human(modelCharaId: 201);
        var backingA = Human(modelCharaId: 202);
        var oldC = Human(modelCharaId: 203);
        var requested = new List<ActorRepresentationKey>();
        transition.RecordApplied(Key(10), source, 1);
        transition.UpdatePublishedAppearance(source, true, 2);

        Assert.True(NativeDrawObjectInjector.TryPreparePersistentTransfer(
            transition,
            destination,
            key =>
            {
                requested.Add(key);
                return (2, key == source ? latest : selectedB);
            },
            out var actor,
            out var prepared));

        Assert.Equal(Key(10), actor);
        Assert.Same(latest, prepared);
        Assert.NotSame(selectedB, prepared);
        Assert.NotSame(backingA, prepared);
        Assert.NotSame(oldC, prepared);
        Assert.Equal(new[] { source }, requested);
        Assert.Equal(destination, transition.CurrentRepresentation);
        Assert.False(transition.IsArmed);
        Assert.False(NativeDrawObjectInjector.TryPreparePersistentTransfer(
            transition,
            destination,
            _ => throw new InvalidOperationException("Duplicate observation must not read a copy."),
            out _,
            out _));
    }

    [Fact]
    public void EarlyKeyC1PartialAndC2UseOnlyTheLatestSourcePublication()
    {
        var transition = new LocalPlayerAppearancePersistence();
        var source = Field(10);
        var early = Cutscene(11);
        var c1Destination = GPose(12);
        var unarmedDestination = Field(13);
        var c2Destination = Cutscene(14);
        var selectedB = Human(modelCharaId: 201);
        var backingA = Human(modelCharaId: 202);
        var oldC = Human(modelCharaId: 203);
        var c1 = Human(modelCharaId: 301);
        var partial = Human(modelCharaId: 302, modelScale: null);
        var c2 = Human(modelCharaId: 303);
        transition.RecordApplied(Key(10), source, 1);

        Assert.False(NativeDrawObjectInjector.TryPreparePersistentTransfer(
            transition,
            early,
            _ => throw new InvalidOperationException("Unarmed early key must not read a publication."),
            out _,
            out _));
        Assert.Equal(source, transition.CurrentRepresentation);
        ...
        Assert.True(NativeDrawObjectInjector.TryPreparePersistentTransfer(
            ...
            out _,
            out var c1Prepared));
        Assert.Same(c1, c1Prepared);
        Assert.NotSame(selectedB, c1Prepared);
        Assert.NotSame(backingA, c1Prepared);
        Assert.NotSame(oldC, c1Prepared);

        var rearm = new LocalPlayerAppearancePersistence();
        rearm.RecordApplied(Key(10), source, 10);
        rearm.UpdatePublishedAppearance(source, true, 11);
        Assert.False(NativeDrawObjectInjector.TryPreparePersistentTransfer(
            rearm,
            unarmedDestination,
            key =>
            ...
            out _,
            out var partialPrepared));
        Assert.Null(partialPrepared);
        Assert.Equal(source, rearm.CurrentRepresentation);
        Assert.False(rearm.IsArmed);

        rearm.UpdatePublishedAppearance(source, true, 13);
        Assert.False(NativeDrawObjectInjector.TryPreparePersistentTransfer(
            rearm,
            unarmedDestination,
            _ => (13, c2),
        ...
        Assert.True(NativeDrawObjectInjector.TryPreparePersistentTransfer(
            ...
            out _,
            out var c2Prepared));
        Assert.Same(c2, c2Prepared);
        Assert.NotSame(c1, c2Prepared);

        rearm.UpdatePublishedAppearance(c2Destination, true, 14);
        Assert.False(NativeDrawObjectInjector.TryPreparePersistentTransfer(
            rearm,
            c2Destination,
            _ => throw new InvalidOperationException("Duplicate destination must not prepare a transfer."),
            out _,
            out _));
    }
