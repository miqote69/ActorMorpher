...
During Actor Morpher's synchronous `EnableDraw` call, `NativeDrawObjectInjector` hooks the current FFXIVClientStructs `CharacterBase.Create` member-function address and substitutes the operation's ModelChara ID, Customize, ten equipment slots, and desired `CharacterBase.ModelScale`. The injection context is managed data scoped to that call; native pointers are stack-local and are never retained. This avoids ObjectKind mutation while allowing NPC body types and non-Human draw objects to be created with their complete source data. Keeping a global `EnableDraw` hook active caused the local player to remain hidden during apply and is not used.

While a local-player Desired appearance is retained, `NativeCutsceneActorTracker` listens to the current FFXIVClientStructs `CharacterSetupContainer.CopyFromCharacter` member function. Only a cutscene ObjectIndex from 200 through 439 whose explicit source ObjectIndex is 0 is linked to the local player. The retained composition starts with the successful Model Search appearance. Each successful local-player Bulk Outfit actor operation replaces only its ten retained equipment values with the outfit that was actually applied or restored. The existing local Character `EnableDraw` implementation then uses that same current composition for both the linked cutscene copy and the field player recreated after the cutscene. This keeps the active morph's ModelChara ID, Customize, and ModelScale together with the latest outfit instead of restoring stale Model Search equipment. The link is cleared by the actual Character virtual destructor taken from the current local actor's runtime vtable, by appearance restore/logout, or when a later copy overwrites the slot. No actor name, address identity, source draw-object dependency, frame polling, redraw retry, automatic rollback, Penumbra IPC, or copied signature is used.

The Ryne plus local Bulk Outfit path was visually accepted in game on 2026-08-30: Ryne's face and the latest player-selected outfit remained correct inside the cutscene and after returning to the field. Diagnostics recorded `ModelCharaId=2720`, Customize signature `46FC138471BC8419`, `CharacterBase.ModelScale=0.84`, and successful local-player Bulk Outfit application before entry. The diagnostic file reached its configured 64 KB limit before the later cutscene events, so the final cutscene and return verdict is User visual acceptance rather than a complete diagnostic trace.

The production memory layer captures ModelChara ID, the generated Customize array, the generated equipment array, and `CharacterBase.ModelScale`. Writes are staged between current FFXIVClientStructs `DisableDraw` and `EnableDraw` member calls and verified afterward. Model scale is written immediately after draw-object creation and reasserted during finalization; success requires the recreated CharacterBase value to match. The first captured scale remains part of BaseData so Restore returns the original player height. Actor identity is re-resolved before every native access; no pointer is retained between frames. Ryne apply and cutscene persistence were verified in game on 2026-08-30; broader model coverage remains pending.
